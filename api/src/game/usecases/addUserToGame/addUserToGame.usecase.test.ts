...
import { AddUserToGameUsecase } from "./addUserToGame.usecase.ts";
...
describe("AddUserToGameUsecase", () => {
  it("should add a user with a valid uuid and the correct username", async () => {
    ...
    const result = await usecase.execute({ id: game.id }, { username: "Alice" });
  ...
  it("should return notFound when game does not exist", async () => {
    ...
    const result = await usecase.execute({ id: validUuid }, { username: "Alice" });
    expect(result).toBe("not_found");
  });

  it("should return invalidArgs when id is not a uuid", async () => {
    const result = await usecase.execute({ id: "not-a-uuid" }, { username: "Alice" });
